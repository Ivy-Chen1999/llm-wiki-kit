...
## Flat-Vault Variant (optional)

Some vaults use a **single `notes/` folder** instead of category folders, expressing the category via frontmatter (`category: concept|entity|reference|insight`). The starter vault shipped with this kit works this way. When the vault is flat, apply these overrides before running any checks:

| Check | Default behavior | Flat-vault override |
|-------|-----------------|--------------|
| #7 Provenance Drift | checks `^[inferred]` markers + `synthesis/` folder | **SKIP** if the vault does not use provenance markers |
| #9 Visibility Tags | checks `_meta/taxonomy.md` | **SKIP** if the vault does not use visibility tags |
| #10 Misc Promotion Candidates | scans `misc/` folder | **SKIP** if no `misc/` folder exists |
| #11 Synthesis Gaps | looks for pages in `synthesis/` | **ADAPT** — look for concept pairs co-occurring ≥ 3 times with no `category: insight` note connecting them; suggest creating one in `notes/` |

When flat, all notes live in `notes/` only. Categories are expressed via frontmatter `category: concept|entity|reference|insight`, not via folders.

Also verify after every lint run: are `_system/index.md` and `_system/hot.md` up to date? If notes exist in `notes/` that are missing from `index.md`, flag and offer to add them.

**Synthesis Gap behavior (trigger 3 of 3):**
...
> "[[notes/X]] and [[notes/Y]] co-occur N times but have no insight page connecting them — want me to draft one now?"

- If yes → create the insight note using the template in wiki-query/SKILL.md, then update index.md and hot.md
