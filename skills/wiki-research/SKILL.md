...
3. Check `$VAULT/_system/tags.md` — use only the tags in the whitelist when writing new notes

**Source Verification (mandatory before filing any note):**
For every claim you intend to write into a wiki page, you must be able to point to a URL you successfully fetched with `WebFetch`. Search snippets alone are not sufficient. Drop any claim whose source page cannot be verified.
